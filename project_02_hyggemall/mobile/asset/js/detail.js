...
jQuery(document).ready(function(){     
    ...
    jQuery(".detail_order_count_btn").on("click",function(){
		var totalPrice = 0;
        var price = jQuery(".detail_selling_price span").text();

		console.log("price = " + price);
        
        price = price.replace(/[^0-9\.]+/, "");
        totalPrice = (price * jQuery(".detail_order_count").val());
        var pattern = /(-?[0-9]+)([0-9]{3})/;
        while(pattern.test(totalPrice)) {
            totalPrice = totalPrice.toString().replace(pattern,"$1,$2");
        }
        jQuery(".detail_sale_price span").text(totalPrice);
    });
    ...
    jQuery(".article_more_btn").on("click",function(){
        jQuery(this).parents(".detail_article").find(".detail_inner_more").show(600);
    ...
    jQuery(".article_close_btn").on("click",function(){
        jQuery(this).parents(".detail_article").find(".detail_inner_more").hide(600);
...
//        desiredHeight1 = desiredHeight1*(0.01) ;
...
//		jQuery("input:radio[name='rating']").removeClass("ui-radio-on");
		jQuery(".classRemove").removeClass("star_on");

		for(var a = 1; a <= rating; a++){
			jQuery("label[for='rating"+a+"']").removeClass("star_on");
			jQuery("label[for='rating"+a+"']").addClass("star_on");
			jQuery("#rating"+a).removeClass("ui-radio-off");
			jQuery("#rating"+a).addClass("ui-radio-on");
		}
    });
