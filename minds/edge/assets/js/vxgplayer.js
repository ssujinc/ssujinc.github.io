window.vxgplayer = function(id, options){
	...
	if(!window.vxgplayer.players[id]){
		...
		window.vxgplayer.players[id] = new function(id, options){
			...
			var autostart_parameter = options.autostart ? options.autostart : self.player.hasAttribute('autostart') ? '1' : '0';
			var html = ''
			...
			self.module.avsync = options.avsync ? options.avsync : self.player.hasAttribute('avsync');
			...
			function moduleDidLoad(){
				...
				var timerId = setInterval(function() {
						if(is_player_initialized == false && document.getElementById(self.id)) {
							if (self.module.command) {
								console.log('now try to connect to chrome web socket...');
